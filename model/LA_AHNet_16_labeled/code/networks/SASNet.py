...
class SASNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False):
        ...
        self.decoder2 = Decoder2(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual, 0)
        self.decoder3 = Decoder3(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual, 0)
    
    def forward(self, input):
        features = self.encoder(input)
        out_tanh2,out_seg2 = self.decoder2(features)
        out_tanh3,out_seg3 = self.decoder3(features)
        return out_seg3, out_seg2, out_tanh3, out_tanh2

if __name__ == '__main__':
    # compute FLOPS & PARAMETERS
    from ptflops import get_model_complexity_info
    model = SASNet(n_channels=1, n_classes=2, normalization='batchnorm', has_dropout=False).cuda()
