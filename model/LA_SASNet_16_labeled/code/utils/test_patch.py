...
def var_all_case(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA"):
    ...
    for image_path in loader:
        ...
        image = h5f['image'][:]
        label = h5f['label'][:]
        prediction, score_map = test_average_output(model, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
    ...
    print('metric is {}'.format(avg_dice))
    return avg_dice
...
def test_all_case(model_name, num_outputs, model, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=1, nms=0):
    ...
    print('metric is {}'.format(avg_metric))
...
def test_average_output(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    w, h, d = image.shape
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    y = model(test_patch)
                    outputs=y[0:2]
                    y1=F.softmax(outputs[0], dim=1)
                    y2=F.softmax(outputs[1], dim=1)
                    y=(y1+y2)/2.0
                y = y.cpu().data.numpy()
                y = y[0,1,:,:,:]
