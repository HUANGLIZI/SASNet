...
class BraTS2019(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = h5f['image'][:]
        label = h5f['label'][:]
        sample = {'image': image, 'label': label.astype(np.uint8),'name':image_name,'view':2,'img_size':image.shape}
        if self.transform:
...
class RandomView(object):
    ...
    def __call__(self, sample):
        ...
        if   self.dataset_name=="Pancreas_CT" or self.dataset_name=="BraTS2019":#(H,W,D)
            randn=np.random.randint(0,3)
            #print('randn',randn)
            if randn==0:
                image=np.transpose(image,(0,2,1))#(H,D,W) 
                label=np.transpose(label,(0,2,1)) 
            elif randn==1:
                image=np.transpose(image,(2,1,0))#(D,W,H)
